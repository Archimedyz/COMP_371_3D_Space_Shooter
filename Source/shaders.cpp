...
bool PrepareShaderPrograms()
{
    ...
    string sShaderFileNames[] = {"SolidColor.fragmentshader", "SolidColor.vertexshader", "ortho2D.vert",
...
bool CShaderProgram::AddShaderToProgram(CShader* shShader)
{
    ...
    glAttachShader(Renderer::GetShaderProgramID(), shShader->GetShaderID());
...
{
    glLinkProgram(Renderer::GetShaderProgramID());
    int iLinkStatus;
    glGetProgramiv(Renderer::GetShaderProgramID(), GL_LINK_STATUS, &iLinkStatus);
...
void CShaderProgram::DeleteProgram()
{
    if(!bLinked)return;
    bLinked = false;
    glDeleteProgram(Renderer::GetShaderProgramID());
}
...
{
    int iLoc = glGetUniformLocation(Renderer::GetShaderProgramID(), sName.c_str());
